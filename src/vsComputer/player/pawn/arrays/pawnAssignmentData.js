import { pawnAssignments } from "../objects/pawnAssignments.js";

const assignedPawnNameData = [
  pawnAssignments.pawnOne.name,
  pawnAssignments.pawnTwo.name,
  pawnAssignments.pawnThree.name,
  pawnAssignments.pawnFour.name,
  pawnAssignments.pawnFive.name,
  pawnAssignments.pawnSix.name,
  pawnAssignments.pawnSeven.name,
  pawnAssignments.pawnEight.name,
];

export const assignedPawnNames = assignedPawnNameData